...
from lm.config import JaxDistributedConfig
...
def scan_or_loop(
    ...
    stack_args = lambda *args: jnp.stack(args) if not all(arg is None for arg in args) else None
...
def maybe_remat(
    ...
    *,
    ...
    """Calls `jax.remat` with the arguments and the correct policy only if the policy is non-empty."""
    if policy:
