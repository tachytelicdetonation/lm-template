...
from lm.config import Config, register_configs
from lm.dataloader.lm_dataset import dummy_dataset, lm_dataset
from lm.infra.checkpoint import Checkpointer, unify_dict_with_eqx_module
from lm.infra.wandb_utils import WandbLogger
from lm.model.loop import Evaluator, train_on_sequence
from lm.model.sharding import ModelSharding
from lm.model.transformer import LanguageModel
from lm.optimizers import make_optimizer
from lm.utils.jax_utils import eval_shape_and_sharding, get_custom_tqdm, initialize_distibuted, master_log, set_random_seed, tree_rearrange
...
def _main(cfg: Config) -> None:
    ...
    optimizer, optimizer_info = make_optimizer(cfg.training.optimizer)
    ...
    @eqx.filter_jit
    def create_sharded_model_and_state() -> tuple[LanguageModel, eqx.nn.State]:
        model, state = eqx.nn.make_with_state(LanguageModel)(cfg, key=key)
        state = jax.device_put(state, jax.NamedSharding(mesh, P()))
        model = model_sharding.shard_params(model)
        return model, state

    @eqx.filter_jit
    def create_stepped_opt_state(model: LanguageModel) -> OptState:
        """Create optimizer state with correct sharding after having a single update step applied."""
        trainable_params = model.trainable_parameters()
        opt_state = optimizer.init(trainable_params)
        _, opt_state = optimizer.update(trainable_params, opt_state, model.trainable_parameters())
        return opt_state
    ...
    if (continued_run and checkpointer.checkpoint_exists()) or cfg.training.load_part != "none":
        if continued_run and checkpointer.checkpoint_exists():
            ...
            load_part = cfg.training.load_part
            load_checkpointer = Checkpointer(config=cfg, for_saving=False)

        if load_part == "all" and cfg.training.eval_mode:
            load_part = "params"
        ...
        def load_model_weights(model: LanguageModel, out_state) -> LanguageModel:
    ...
    evaluator = Evaluator(
        global_batch_size=max(cfg.training.eval_batch_size, cfg.training.global_batch_size // cfg.training.accum_steps * 4),
    ...
    with mesh:
        ...
        for step in tqdm(range(start_step, total_steps), initial=start_step, total=total_steps, desc="Training", disable=not master_process):
            ...
            update = {
                ...
                "learning_rate": jnp.asarray(optimizer_info["learning_rate_schedule"](int(opt_state[1][2].count) - 1)).item(),
            }
